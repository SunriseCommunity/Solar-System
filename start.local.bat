...
call :print_info "Starting with local compose file: %COMPOSE_FILE%"
echo.
...
if not exist "%COMPOSE_FILE%" (
    set "missing_files[!missing_files_count!]=%COMPOSE_FILE%"
    set /a missing_files_count+=1
    call :print_warning "%COMPOSE_FILE% file not found"
) else (
    call :print_success "%COMPOSE_FILE% file exists"
)
